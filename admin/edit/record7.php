...
function renderForm($one = '', $two ='', $three ='', $error = '', $id = '')
{ ?>
<?php 
...
<body>
	...
	<section class="right" id="right">
			...
			<form action="" method="post">
				...
				<div class="input">
					<strong>Percentage:</strong> <input type="text" name="percent"
					value="<?php echo $two; ?>"/>
				</div>
				<div class="input">
					<strong>Expiration (M/D/Y - 01/01/0001):</strong> <input type="text" name="expiry"
...
$percent = htmlentities($_POST['percent'], ENT_QUOTES);
$expiry = htmlentities($_POST['expiry'], ENT_QUOTES);
...
renderForm($code, $percent, $expiry, $error, $id);
}
else
{
// if everything is fine, update the record in the database
if ($stmt = $mysqli->prepare("UPDATE discountcodes SET code = ?, percent = ?, expiry = ?
WHERE id=?"))
{
$stmt->bind_param("sssi", $code, $percent, $expiry, $id);
$stmt->execute();
$stmt->close();
}
...
header("Location: ../index.php?name=seven");
}
}
...
if($stmt = $mysqli->prepare("SELECT * FROM discountcodes WHERE id=?"))
{
$stmt->bind_param("i", $id);
$stmt->execute();

$stmt->bind_result($id, $code, $percent, $expiry);
$stmt->fetch();

// show the form
renderForm($code, $percent, $expiry, NULL, $id);
...
// if they are empty, show an error message and display the form
$error = 'ERROR: Please fill in all required fields!';
renderForm($code, $percent, $expiry, $error);
}
else
{
// insert the new record into the database
if ($stmt = $mysqli->prepare("INSERT discountcodes (code, percent, expiry) VALUES (?, ?, ?)"))
{
$stmt->bind_param("sss", $code, $percent, $expiry);
$stmt->execute();
$stmt->close();
}
// show an error if the query has an error
